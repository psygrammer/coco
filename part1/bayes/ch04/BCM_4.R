...
# Normal Distribution
# f(x) = 1/(?ˆš(2 ?€) ??) e^-((x - Î¼)^2/(2 ??^2)) 
theta = rnorm(n=1000, mean=0, sd=5)   # ?°?´?„° ?ƒ?„±
summary(theta)
par(mfrow=c(1,2))
...
# ?‰ê· í‚¤ê°€ 175?´ê³? ?‘œì¤€?Ž¸ì°¨ê?€ 5?¸ ?‚¬?žŒ?“¤?— ??€?•œ ë³€?ˆ˜ ?ƒ?„±?•˜ê³? Graph 
height=rnorm(n=10000,mean = 175, sd=5)   # ?°?´?„° ?ƒ?„±
plot(density(height))
hist(height, breaks=20, probability=TRUE)    # ?‚¬ì§?1
hist(height, breaks=100, probability=TRUE)   # ?‚¬ì§?2
hist(height, breaks=300, probability=TRUE)   # ?”?š± ?” ?™•ë¥ ë?€?„ ?•¨?ˆ˜?— ê°€ê¹Œìš´ ?žˆ?Š¤?† ê·¸ëž¨
...
# ? •?•´ì§? ?‹œê°? ?•ˆ?— ?–´?–¤ ?‚¬ê±´ì´ ?¼?–´?‚  ?šŸ?ˆ˜?— ??€?•œ ê¸°ëŒ“ê°’ì„ Î»(lambda)?¼ê³? ?–ˆ?„ ?•Œ, ê·? ?‚¬ê±´ì´ x?šŒ ?¼?–´?‚  ?™•ë¥?
# p(x) = Î»^x exp(-Î»)/x!
# for x = 0, 1, 2, ?€? . The mean and variance are E(X) = Var(X) = Î». 
...
# Gamma Distribution
#  ?—°?† ?™•ë¥ ë¶„?¬ë¡?, ?‘ ê°œì˜ ë§¤ê°œë³€?ˆ˜ë¥? ë°›ìœ¼ë©? ?–‘?˜ ?‹¤?ˆ˜ë¥? ê°€ì§? ?ˆ˜ ?žˆ?‹¤.
? dgamma
