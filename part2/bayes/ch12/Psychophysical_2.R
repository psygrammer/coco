...
setwd("D:/Work/인지모델링/source_code/CaseStudies/PsychophysicalFunctions")
library(R2WinBUGS)
# WinBUGS 설치 디렉토리
...
for (i in 1:8) {
	...
	print(ggplot(data=x_rprop,aes(x=xv,y=rpropv)) + geom_point()+geom_smooth())
	par(ask=TRUE)	
}
...
for (i in 1:nsubjs)
{
	plot(density(JND[,i]), col="red", type="h", main=paste("Subject",as.character(i)), ylab="", xlab="", xlim=c(10,100), ylim=c(0,0.12), las=1,yaxt="n",xaxt="n")
	par(new=T)
	plot(density(JND2[,i]), col="blue",type="h", main="",ylab="",xlab="",xlim=c(10,100),ylim=c(0,0.12),yaxt="n",xaxt="n",bty="n")
	par(new=F)
	if (i==1 | i==5) axis(2,las=1, yaxp=c(0,0.12,3))
	if (i>4) axis(1)
	if (i==4) legend("topright",c("non-contaminant model","contaminant model"),col=c("red","blue"),bty="n",pch=c(15,15))
}
